...
class NumberPassedQuestion(Question):
    ...
    # grades.addPoints(qs.count(True))
    # changed to enable partial credit that is proportional to 
    # number of test cases passed

    def execute(self, grades):
        qs = [f(grades) for _, f in self.testCases]
        grades.assignZeroCredit()
        grades.addPoints(round(6.25 * qs.count(True)))



class NumberPassedQuestion2(Question):
    """Grade is the number of test cases passed."""
    # this was the previous line:
    # grades.addPoints(qs.count(True))
    # changed to enable partial credit that is proportional to 
    ...
    def execute(self, grades):
        qs = [f(grades) for _, f in self.testCases]
        grades.assignZeroCredit()
        grades.addPoints(round(8.33333 * qs.count(True)))
